...
/* This correctly hides the input with aria-label="internal_bridge" */
...
# --- BRIDGE LOGIC (Fixing the crash & Double Input issue) ---
# Logic Step 1: Check if the bridge has a value (JS sent something)
if st.session_state.venue_bridge:
    ...
    # Logic Step 2: Clear the state immediately so it's ready for next time
    st.session_state.venue_bridge = ""

# Logic Step 3: Render the input AFTER clearing the state.
# We give it a specific aria-label so our CSS can hide it securely.
    ...
    if not st.session_state.mis_no:
        ...
        if subs:
            ...
            if table:
                ...
                if st.sidebar.button("Refresh Data / Clear Cache"): st.cache_data.clear(); st.rerun()
            ...
            with col_daily_list:
                ...
                if not daily_classes: st.info("😴 No classes scheduled for this day.")
                else:
                    ...
                    for i, cls in enumerate(daily_classes):
                        ...
                        with c_info: st.markdown(f"""<div class="daily-card" style="border-left: 5px solid {border_color};"><div class="daily-info"><h4>{cls['Subject']}</h4><p>⏰ {cls['StartTime']} • {cls['Type']} • 📍 {cls['Venue']}</p></div></div>""", unsafe_allow_html=True)
                        with c_action:
                            ...
                            if st.button(btn_label, key=cls_id, type=btn_type, use_container_width=True):
                                if is_present: del st.session_state.attendance[cls_id]; update_attendance_in_sheet(cls_id, "remove")
                                else: st.session_state.attendance[cls_id] = True; update_attendance_in_sheet(cls_id, "add")
                                st.rerun()
            ...
            for sub_key, total_count in total_possible.items():
                ...
                for att_id in st.session_state.attendance:
                    ...
                    if len(parts) >= 5 and parts[0] == mis and parts[2] == subject_name and parts[3] == subject_type: attended += 1
                ...
                msg_color = "#2ecc71"
                if percentage < 60: border_grad = "linear-gradient(135deg, #ff9a9e, #fecfef)"; bg_color = "rgba(255, 0, 0, 0.05)" if is_dark else "#fff5f5"; msg_color = "#e74c3c"
                elif percentage < 75: border_grad = "linear-gradient(135deg, #f6d365, #fda085)"; bg_color = "rgba(255, 165, 0, 0.05)" if is_dark else "#fffdf5"; msg_color = "#e67e22"
...
st.markdown(f"""<div style="text-align: center; margin-top: 50px; font-size: 13px; color: {footer_color};">Student Portal © 2026 • Built by <span style="color:#6a11cb; font-weight:700">IRONDEM2921 [AIML]</span></div>""", unsafe_allow_html=True)

# --- JS INJECTION (Updated selector) ---
components.html("""
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function triggerStreamlitUpdate(day, time) {
            // Find input with aria-label="internal_bridge"
            const bridgeInput = window.parent.document.querySelector('input[aria-label="internal_bridge"]');
        ...
        emptySlots.forEach(slot => {
            slot.addEventListener('dblclick', function() { triggerStreamlitUpdate(this.dataset.day, this.dataset.time); });
            slot.addEventListener('touchstart', function(e) {
                if (e.touches.length === 1) { pressTimer = setTimeout(() => { triggerStreamlitUpdate(this.dataset.day, this.dataset.time); }, 800); }
            });
